...
#ifndef PHLOXAR_FILTERS_HPP
#define PHLOXAR_FILTERS_HPP
...
inline npy_intp fix_offset(const EXTENDMode mode, npy_intp cc, const npy_intp len) {
...
struct filter_iterator {
  ...
  // to the filter offsets when moving into a different region in the array.
  filter_iterator(PyArrayObject* array, PyArrayObject* filter,
                  EXTENDMode mode=EXTENDNearest, bool compress=true)
  : filter_data_(numpy::ndarray_cast<T*>(filter)),
    own_filter_data_(false), nd_(PyArray_NDIM(array)) {
  ...
  const T*operator [](const npy_intp j) const {
    assert(j < size_);
    return filter_data_[j];
  }
