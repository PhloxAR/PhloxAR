...
def mf_tracker(img, bbox, ts, oldimg, **kwargs):
    ...
    * *bbox*  - tuple - Bounding Box tuple (x, y, w, h)
    ...
    **HOW TO USE**
    ...
    >>> ts = []
    >>> img = cam.getImage()
    >>> bbox = (100, 100, 300, 300) # get BB from somewhere
    >>> ts = mf_tracker(img, bbox, ts, img, numM=15, numN=15, winsize=12)
    ...
    oldg = oldimg.gray_narray
    newg = img.gray_narray
    bbox = [bbox[0], bbox[1], bbox[0] + bbox[2], bbox[1] + bbox[3]]
    bbox, shift = fbtrack(oldg, newg, bbox, numM, numN, margin, winsize_ncc,
                          winsize_lk)
    bbox = [bbox[0], bbox[1], bbox[2] - bbox[0], bbox[3] - bbox[1]]
    track = MFTrack(img, bbox, shift)
    return track


def fbtrack(imgI, imgJ, bbox, numM=10, numN=10, margin=5, winsize_ncc=10,
            winsize_lk=4):
    """
    **SUMMARY**
    (Dev Zone)
    ...
    **PARAMETERS**
    ...
    bbox - Bounding box represented through 2 points (x1,y1,x2,y2)
    ...
    **RETURNS**
    ...
    scaleshift - relative scale change of bbox

    """

    nPoints = numM * numN
    sizePointsArray = nPoints * 2
    # print bbox, "passed in fbtrack"
    pt = get_filled_bbox_points(bbox, numM, numN, margin)
    ...
    medFb = get_median(fbLKCleaned)
    medNcc = get_median(nccLKCleaned)
    ...
    newBB, scaleshift = predict_bbox(bbox, startPoints, targetPoints, nAfterFbUsage)
...
def lktrack(img1, img2, ptsI, nPtsI, winsize_ncc=10, win_size_lk=4,
    ...
    **RETURNS**
    ...
    ncc - norm_cross_correlation values
    ...
    ncc = norm_cross_correlation(img1, img2, template_pt, target_pt, status,
                                 winsize_ncc, method)
...
def get_filled_bbox_points(bbox, numM, numN, margin):
    """
    ...
    **RETURNS**
    ...
    """
    pointDim = 2
    bb_local = (bbox[0] + margin, bbox[1] + margin, bbox[2] - margin, bbox[3] - margin)
    if numM == 1 and numN == 1:
        pts = calculate_bbox_center(bb_local)
        return pts
        ...
        center = calculate_bbox_center(bb_local)
        pt = [0.0] * (2 * numM * numN)
        for i in range(numN):
            for j in range(numM):
                pt[i * numM * pointDim + j * pointDim + 0] = center[0]
                pt[i * numM * pointDim + j * pointDim + 1] = bb_local[
                                                                 1] + j * spaceM
        ...
        for i in range(numN):
            for j in range(numN):
                pt[i * numM * pointDim + j * pointDim + 0] = bb_local[
                                                                 0] + i * spaceN
...
def predict_bbox(bbox0, pt0, pt1, nPts):
    """
    ...
    **PARAMETERS**
    ...
    bbox0 - Bounding Box represented through 2 points (x1,y1,x2,y2)
    pt0 - Starting Points
    pt1 - Target Points
    ...
    **RETURNS**
    ...
    bbox1 - new bounding box
    shift - relative scale change of bbox0
    ...
    dx = get_median_unmanaged(ofx)
    dy = get_median_unmanaged(ofy)
    ofx = ofy = 0
    ...
    shift = get_median_unmanaged(dist0)
    if shift is None:
        return bbox0, 1.0
    ...
    if shift > 1.1 or shift < 0.9:
        shift = 1
    s0 = 0.5 * (shift - 1) * get_bbox_width(bbox0)
    s1 = 0.5 * (shift - 1) * get_bbox_height(bbox0)
...
def euclidean_distance(point1, point2):
    ...
    **RETURNS**
    ...
    match = returns a vector of euclidean distance
    """
